...
public class AddCardFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Button addNewCard = v.findViewById(R.id.addNewCard);

        addNewCard.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                EditText front = v.findViewById(R.id.newFront);
                EditText back = v.findViewById(R.id.newBack);

                String frontS = front.getText().toString();
                String backS = back.getText().toString();


                if (!frontS.isEmpty() && !backS.isEmpty()) {
                    Card c = new Card(frontS, backS);
                    DeckConfiguration dc = (DeckConfiguration) getActivity();
                    dc.addCardToCurrentDeck(c);
